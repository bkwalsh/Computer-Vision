...
def conv_2d(image, filt, mode='zero'):
   ...
   fX, fY = filt.shape
   rX, rY = math.ceil((fX - 1) / 2), math.ceil((fY - 1) / 2)
...
def denoise_gaussian(image, sigma = 1.0):
   rS = math.ceil(3 * sigma)
   rows, cols = image.shape
   result = np.zeros_like(image)
   padded = mirror_border(image, wx = rS, wy = rS)
   gauss = gaussian_1d(sigma)

   for i in range(rS, rows + rS):
      for j in range(rS, cols + rS):
         pointRange = padded[i - rS: i + rS + 1, j - rS: j + rS +1]
         first_gauss = oneDconvolution(pointRange, gauss)
         second_gauss = oneDconvolution(first_gauss, gauss.T)
         result[i - rS,j - rS] = second_gauss
...
def sobel_gradients(image):
   ...
   padded = mirror_border(image, wx = 2, wy = 2)
   rows, cols = image.shape

   for i in range(0,rows):
      for j in range(0,cols):
         pointRange = padded[i + 1: i + 4, j + 1: j + 4]
         dx[i,j] = oneDconvolution(oneDconvolution(pointRange,cB),cA.T)
         dy[i,j] = oneDconvolution(oneDconvolution(pointRange,cA),cB.T) 
